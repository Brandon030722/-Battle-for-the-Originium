...
class CardGameGUI(tk.Tk):
    ...
    def show_card_on_deployment(self, img_path, card_name, cost, attack, health, is_enemy):
        ...
        print(f"Deployed {card_name} on {'enemy' if is_enemy else 'ally'} deployment area.")
...
######################################################################################################################
    ...
    def attack_card(self):
        available_cards = self.frontline_cards if self.player_number == 1 else self.enemy_deployed_cards
        if not available_cards:
            print("No available cards to attack with.")
            return

        print("Select your card to attack with:")
        for idx, card_data in enumerate(available_cards):
            if not self.attacked_this_turn.get(card_data['image_id'], False):
                print(f"{idx + 1}. {card_data['name']} (Attack: {card_data['attack']})")

        card_index = int(input("Enter card number: ")) - 1
        if card_index < 0 or card_index >= len(available_cards) or self.attacked_this_turn.get(
                available_cards[card_index]['image_id'], False):
            print("Invalid choice or card has already attacked this turn.")
            return

        attacking_card = available_cards[card_index]
        self.attacked_this_turn[attacking_card['image_id']] = True  # 标记为已攻击

        # 确定可以攻击的目标
        target_cards = self.deployed_cards if self.player_number == 1 else self.frontline_cards
        print("Select a target card to attack:")
        for idx, card_data in enumerate(target_cards):
        ...
        target_card = target_cards[target_index]
        # 执行攻击
        target_card['health'] -= attacking_card['attack']
        if target_card['health'] <= 0:
            target_cards.pop(target_index)  # 目标卡牌被销毁
